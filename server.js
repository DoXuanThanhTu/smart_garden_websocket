...
const deviceThresholds = {}; // { deviceId: { temp:{min,max}, humidity:{min,max}, soil:{min,max}, pump:true/false } }
...
// Set threshold
function setThreshold(deviceId, thresholds) {
  if(!deviceThresholds[deviceId]) deviceThresholds[deviceId] = {};
  deviceThresholds[deviceId].temp = thresholds.temp || deviceThresholds[deviceId].temp;
  deviceThresholds[deviceId].humidity = thresholds.humidity || deviceThresholds[deviceId].humidity;
  deviceThresholds[deviceId].soil = thresholds.soil || deviceThresholds[deviceId].soil;
  if(thresholds.pump !== undefined) deviceThresholds[deviceId].pump = thresholds.pump;

  const ws = devices[deviceId];
  if(ws && ws.readyState === WebSocket.OPEN){
    ws.send(JSON.stringify({ action: 'setThresholds', thresholds }));
    throttleLog(`[${new Date().toISOString()}] Sent thresholds to ${deviceId}`);
  }
}

// Set pump directly
function setPump(deviceId, pumpState) {
  if(!deviceThresholds[deviceId]) deviceThresholds[deviceId] = {};
  deviceThresholds[deviceId].pump = pumpState;

  const ws = devices[deviceId];
  if(ws && ws.readyState === WebSocket.OPEN){
    ws.send(JSON.stringify({ action: 'controlPump', pump: pumpState }));
    throttleLog(`[${new Date().toISOString()}] Sent pump=${pumpState} to ${deviceId}`);
  }
}

// Auto control pump based on sensor data
function autoControlPump(deviceId, sensorData) {
  const thresholds = deviceThresholds[deviceId];
  if(!thresholds) return;

  let pumpState = thresholds.pump ?? false;

  if(thresholds.temp){
    if(sensorData.temp < thresholds.temp.min) pumpState = true;
    if(sensorData.temp > thresholds.temp.max) pumpState = false;
  }
  if(thresholds.humidity){
    if(sensorData.humidity < thresholds.humidity.min) pumpState = true;
    if(sensorData.humidity > thresholds.humidity.max) pumpState = false;
  }
  if(thresholds.soil){
    if(sensorData.soil < thresholds.soil.min) pumpState = true;
    if(sensorData.soil > thresholds.soil.max) pumpState = false;
  }

  setPump(deviceId, pumpState);
}
    ...
    if(type === 'esp32'){
      ...
      ws.on('message', message => {
        try {
          ...
          if(data.action === 'setThresholds') setThreshold(data.deviceId, data.payload);
          if(data.action === 'controlPump') setPump(data.deviceId, data.payload.pump);

        } catch(err){
          throttleLog('Invalid JSON from dashboard:', message.toString());
        }
      });
